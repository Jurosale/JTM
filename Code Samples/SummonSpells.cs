...
public class SummonSpells : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        if (other.tag == "Player")
        {
            ...
            if (canWater && (speech.word == "water" || uiH.isWater))
            {
                ...
                //rainDrops.transform.position = new Vector3(GetComponent<Collider2D>().bounds.center.x, GetComponent<Collider2D>().bounds.max.y + 4f, -30f);
                waterFlag = false;
    ...
    void FixedUpdate()
    {
        ...
        if (canIce && (speech.word == "ice" || uiH.isIce))
        {
            ...
            //Debug.Log(Vector2.Distance(avatar.transform.position, transform.position));
            //Debug.Log(avatar.GetComponent<Collider2D>().bounds.min.y + " vs "  + transform.position.y);
            if (Vector2.Distance(avatar.transform.position, transform.position) > 15f) {
                //GetComponent<FMODUnity.StudioEventEmitter>().TriggerOnce = false;
                transform.localScale = new Vector3(1.0f, 0.0f, 1.0f);
            }
            else if (avatar.GetComponent<Collider2D>().bounds.min.y < transform.position.y)
            {
                //GetComponent<FMODUnity.StudioEventEmitter>().TriggerOnce = true;
                if (iceFlag)
                {

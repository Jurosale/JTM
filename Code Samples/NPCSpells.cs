...
public class NPCSpells : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        prayAnim = false;
        shrineTimer = 0f;
        //spiritspeech.GetComponent<TextMesh>().text = spiritTitle;
        //spiritMessage = spiritMessage.Replace("\\n", "\n");
    ...
    void Update()
    {
        ...
        if (speech.word == "speak" && canPray == true && !followAnim)
        {
            speech.word = "";
            speech.castOn = false;
            uiH.isSpell = true;
        ...
        if (speech.word == "follow" && canPray == true && isPuzzle == true)
        {
            //play anim
            followAnim = true;
        }
        ...
        if (speech.word == "prayer" && canPray == true && isPuzzle == true)
        {
        ...
        if (prayAnim == true)
        {
            ...
            //Start Prayer reverb
            //reverbPrayer = FMODUnity.RuntimeManager.CreateInstance(snapshotPrayer);
            //reverbPrayer.start();
        ...
        if (prayTimer > 10.0f)
        {
            ...
            //if (Mathf.Abs(transform.position.y - vivi.transform.position.y) < 30f) {
            uiH.isSpell = false;
            Destroy(gameObject);
            //}
            ...
            //Stops the reverb
            //reverbPrayer.stop(FMOD.Studio.STOP_MODE.ALLOWFADEOUT);
